-- CREATE 
-- 테이블 생성, 데이터베이스의 객체를 생성하는 DDL

CREATE TABLE MEMBER ( --컬럼만들기
	MEMBER_NO NUMBER,
...
-- 제약조건
-- NOT NULL: '널 값 허용하지 않겠다.' 
-- 반드시 값을 기록해야하는 컬럼에 해당 제약조건을 등록.
...
-- 테이블 값에 추가하기
-- INSERT => DML (데이터를 조작할 수 있다.)
INSERT INTO USER_NOCONS VALUES(1, 'USER01','PASS01','김철수');
...
INSERT INTO USER_NOT_NULL VALUES(1,'USER01','PASS01','김철수');
INSERT INTO USER_NOT_NULL VALUES(2,NULL,NULL,NULL);-- 오류뜸 .. NULL값 받아서

SELECT * FROM USER_NOT_NULL;

-- NAME은 NOT NULL 제약조건을 등록하지 않았기에 NULL값이 잘 들어간다.
INSERT INTO USER_NOT_NULL VALUES(2, 'USER02','PASS02',NULL);


-- UNIQUE 제약조건 --
-- 중복을 허용하지 않는 제약조건
-- 컬럼에 값을 입력/ 수정할때 중복 확인하여
-- 만약 값이 있을 경우 수정 및 추가를 못하는 제약이다.
--
...
VALUES(1,'USER01','PASS01','김철수');

INSERT INTO USER_UNIQUE;
VALUES(1,'USER02','PASS01','김철수');
...
CREATE TABLE USER_UNIQUE2(
	...
	USER_PWD VARCHAR2(30),
	USER_NAME VARCHAR2(15),
	UNIQUE(USER_ID) -- 컬럼이 모두 작성된 후 에 별도로 작성하는 제약조건: '테이블레벨 제약조건'
);

INSERT INTO USER_UNIQUE2 VALUES(1,'USER01','PASS01','김철수');
...
CREATE TABLE USER_UNIQUE3(
	...
	USER_PWD VARCHAR2(30),
	USER_NAME VARCHAR2(15),
	UNIQUE(USER_NO,USER_ID)--두 개 이상의 컬럼을 중복되지 않게 설정하게
);

SELECT  * FROM USER_UNIQUE3;

INSERT INTO USER_UNIQUE3 VALUES(1,'USER01','PASS01','김철수');
INSERT INTO USER_UNIQUE3 VALUES(1,'USER02','PASS02','김바보');
INSERT INTO USER_UNIQUE3 VALUES(2,'USER01','PASS03','이바보');
...
-- CHECK --
-- 컬럼에 값을 기록할때 지정한 값 이외에는 값이 저장되지 않도록 범위를 제한하는 조건
...
INSERT INTO USER_CHECK2 VALUES(-10);--에러
...
-- 테이블 전체에대한 각 데이터(ROW)의 식별자 역할을 수행시키는 제약조건.
-- 값이 반드시 들어가고, 중복이 있어서도 안된다.

-- 테이블마다 기본키 제약조건은 하나만 존재. (하나의 컬럼에만 기본키 제약조건을 설정가능)
-- PRIMARY KEY는 한 컬럼에 적용가능하고, 여러 컬럼을 묶어서도 적용가능..


CREATE TABLE USER_PK_TABLEE(
...
SELECT * FROM USER_CONSTRAINTS
WHERE TABLE_NAME = 'USER_PK_TABLEE';

INSERT INTO USER_PK_TABLEE
VALUES(1, 'USER01','PASS01','김철수','남');

SELECT * FROM USER_PK_TABLEE;

INSERT INTO USER_PK_TABLEE
VALUES(2, 'USER02','PASS02','박바보','여');

INSERT INTO USER_PK_TABLEE --오류 
VALUES(1, 'USER03','PASS03','이바부','여');

INSERT INTO USER_PK_TABLEE --오류 
VALUES(NULL, 'USER03','PASS03','이바부','여');

SELECT * FROM USER_PK_TABLE;
-- 기본키 제약조건을 설정하면 NOT NULL과 UNIQUE가 함께 설정된다.

-- 기본키 제약조건을 여러 칼럼에 적용하기
...
-- 회원 아이디는 중복불가에 필수 입력사항,
-- 회원 비밀번호는 필수 입력사항,
-- 회원이름,
-- 성별('F','M')만 입력 받을 수 있게,
-- 회원 연락처
-- 이러한 회원의 정보를 저장할 수 있는 컬럼을 가진 MEMBER 테이블 생성.
-- 컬럼의 크기는 편하신대로, 테이블 생성 후에는 INSERT 5번 이상 진행후, SELECT까지 확인

DROP TABLE MEMBER;
-- MEMBER라는 같은 이름의 테이블이 있으면 DROP으로 삭제 후 진행.
CREATE TABLE MEMBER(
...
INSERT INTO MEMBER VALUES(1,'USER01','PASS01','김길동','F','010-1111-2222');
INSERT INTO MEMBER VALUES(2,'USER02','PASS01','이길동','M','010-2222-2222');
INSERT INTO MEMBER VALUES(3,'USER03','PASS03','양길동','M','010-3333-2222');
INSERT INTO MEMBER VALUES(4,'USER04','PASS04','박길동','F','010-4444-2222');
INSERT INTO MEMBER VALUES(5,'USER05','PASS05','최길동','F','010-5555-2222');

-- FOREIGN KEY
-- 외래키, 외부키, 참조키
-- 다른 테이블의 컬럼값을 참조(REFERENCE)하여 참조하는 테이블 값에 접근
-- FOREIGN KEY 제약조건을 총해 테이블들 간에 관계(RELATIONSHIP)형성된다.

-- 컬럼레벨
-- 컬럼명 자료형(길이) REFERENCE 참조할 테이블명[(참조할 컬럼명)] [삭제옵션]
-- 테이블레벨
-- [CONSTRAINT 제약조건명] FOREIGN KEY (적용할 컬럼명) REFERENCES 참조할 테이블명[(참조할 컬럼명)] [삭제옵션]
-- 참조할 컬럼명을 생략하면 참조할 테이블의 기본키(PK)를 참조할 컬럼으로 가져온다.
-- 참조하고자 하는 컬럼은 반드시 PK이거나 UNIQUE 제약조건이 걸려있어야한다.
...
-- 참조하고 있는 원본테이블(부모) 컬럼 값이 삭제 될때
-- 참조한 값을 어떻게 처리할지 설정하는 옵션 

-- 일반적으로 참조하는 원본 컬럼의 내용을 삭제하려 한다면
-- 외래키로 사용중인 자식 컬럼이 존재할때는 함부러 삭제 불가능하다..
-- 함부로 삭제할 수 없다. 이를 변경하기 위한 옵션

-- 1. 부모 컬럼을 삭제할때 자식을 NULL로 변경하기.
-- ON DELETE SET NULL
...
AS SELECT * FROM EMPLOYEE WHERE 1 = 0; -- 1==0 ? 만족할 일이 없음..
...
-- ALTER TABLE 테이블명 ADD FOREIGN KEY(컬럼명) REFERENCES 참조할 테이블 명(컬럼명)
...
--제약조건을 중간에 변경 할 경우 이미 저장한 값등을 파악하여 제약조건에 위배되지 않는지 확인해야한다.
...
ON(C1.TABLE_NAME = C2.TABLE_NAME AND C1.CONSTRAINTS_NAME = C2.CONSTRAINTS_NAME)
